...
module Innodb
  class LogRecord
    ...
    def read_preamble(cursor)
      type_and_flag = cursor.name('type') { cursor.get_uint8 }
      ...
      when :MULTI_REC_END, :DUMMY_RECORD
        ...
        Preamble.new(
          ...
          space: cursor.name('space') { cursor.get_ic_uint32 },
          page_number: cursor.name('page_number') { cursor.get_ic_uint32 }
        )
      end
    end
    ...
    def read_index(cursor)
      n_cols = cursor.name('n_cols') { cursor.get_uint16 }
      n_uniq = cursor.name('n_uniq') { cursor.get_uint16 }
      cols = n_cols.times.collect do
        info = cursor.name('field_info') { cursor.get_uint16 }
        IndexFieldInfo.new(
    ...
    def read_insert_record(cursor)
      page_offset = cursor.name('page_offset') { cursor.get_uint16 }
      end_seg_len = cursor.name('end_seg_len') { cursor.get_ic_uint32 }

      if (end_seg_len & INFO_AND_STATUS_MASK) != 0
        info_and_status_bits = cursor.get_uint8
        origin_offset = cursor.get_ic_uint32
        mismatch_index = cursor.get_ic_uint32
      end

      {
        ...
        record: cursor.name('record') { cursor.get_bytes(end_seg_len >> 1) },
      }
    end
    ...
    def read_update_in_place_record(cursor)
      {
        flags: cursor.name('flags') { cursor.get_uint8 },
        sys_fields: read_sys_fields(cursor),
        rec_offset: cursor.name('rec_offset') { cursor.get_uint16 },
    ...
    def read_update_index(cursor)
      info_bits = cursor.name('info_bits') { cursor.get_uint8 }
      n_fields  = cursor.name('n_fields') { cursor.get_ic_uint32 }
      fields = n_fields.times.collect do
        {
          field_no: cursor.name('field_no') { cursor.get_ic_uint32 },
          len: len = cursor.name('len') { cursor.get_ic_uint32 },
          data: cursor.name('data') { len != LENGTH_NULL ? cursor.get_bytes(len) : :NULL },
        }
      end
      {
        info_bits: info_bits,
    ...
    def read_sys_fields(cursor)
      {
        trx_id_pos: cursor.name('trx_id_pos') { cursor.get_ic_uint32 },
        roll_ptr: cursor.name('roll_ptr') { cursor.get_bytes(7) },
        trx_id: cursor.name('trx_id') { cursor.get_ic_uint64 },
      }
    end
    ...
    def read_clust_delete_mark(cursor)
      {
        flags: cursor.name('flags') { cursor.get_uint8 },
        value: cursor.name('value') { cursor.get_uint8 },
    ...
    def read_payload(type, cursor)
      ...
      when :MLOG_1BYTE, :MLOG_2BYTE, :MLOG_4BYTE
        {
          page_offset: cursor.name('page_offset') { cursor.get_uint16 },
          value: cursor.name('value') { cursor.get_ic_uint32 },
        }
      when :MLOG_8BYTE
        {
          offset: cursor.name('offset') { cursor.get_uint16 },
          value: cursor.name('value') { cursor.get_ic_uint64 },
        }
      ...
      when :UNDO_INSERT
        {
          length: length = cursor.name('length') { cursor.get_uint16 },
          value: cursor.name('value') { cursor.get_bytes(length) },
        }
      when :REC_INSERT
        {
      ...
      when :UNDO_INIT
        {
          type: cursor.name('type') { UNDO_TYPES[cursor.get_ic_uint32] },
        }
      when :FILE_CREATE, :FILE_DELETE
        {
          name_len: name_len = cursor.name('name_len') { cursor.get_uint16 },
          name: cursor.name('name') { cursor.get_bytes(name_len) },
        }
      when :FILE_CREATE2
        {
          flags: cursor.name('flags') { cursor.get_uint32 },
          name_len: name_len = cursor.name('name_len') { cursor.get_uint16 },
          name: cursor.name('name') { cursor.get_bytes(name_len) },
        }
      when :FILE_RENAME
      ...
      when :LIST_END_COPY_CREATED
        {
          length: length = cursor.name('length') { cursor.get_uint32 },
          data: cursor.name('data') { cursor.get_bytes(length) },
        }
